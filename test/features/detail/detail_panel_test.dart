...
void main() {
  Future<void> pumpPanel(WidgetTester tester, DetailPanel panel) {
    return tester.pumpWidget(
      MaterialApp(home: Scaffold(body: Center(child: panel))),
    );
  }

  testWidgets('wears the `.panel` surface — card fill, 16px radius, line border',
  ...
  testWidgets('shows the heading in caps but speaks it in natural case',
    ...
    expect(
      find.bySemanticsLabel('How big is it? — car-sized'),
      findsOneWidget,
    );
    handle.dispose();
  });
